...
def load_ff(model):
    if model == 'Protein':
        path1 = pkg_res.resource_filename("iConBuilder", "forcefield/top_protein.inp")
        path2 = pkg_res.resource_filename("iConBuilder", "forcefield/param_protein.inp")
    elif model == 'RNA':
        path1 = pkg_res.resource_filename("iConBuilder", "forcefield/top_RNA.inp")
        path2 = pkg_res.resource_filename("iConBuilder", "forcefield/param_RNA.inp")
    else:
...
def setup(args, dt, pressure=1*unit.atmosphere, friction=0.1/unit.picosecond, gpu_id="0"):
    ...
    print('\n################## build system ###################')
    system = buildiConSystem(psf, system, ffs)
